...
## 시연 영상
![CineMatch Demo](docs/assets/cinematch_demo.webp)
...
CineMatch는 모델의 성능을 다각도로 분석하기 위해 **정확도(Relevance)**와 **비즈니스 가치(Business Value)** 두 측면의 지표를 모두 사용합니다. 입문자분들을 위해 각 지표의 의미를 쉽게 풀이하면 다음과 같습니다.

### 1. 정확도 지표 (사용자가 좋아할 영화를 잘 맞췄는가?)
- **적중률 (HitRate@K)**: 추천된 상위 K개의 영화 중 사용자가 실제로 과거에 시청했거나 좋아했던 영화가 **최소 한 개 이상 포함될 확률**입니다. 가장 직관적인 성능 지표입니다.
- **순위 가중치 (NDCG@K)**: 단순히 맞추는 것을 넘어, 사용자가 더 좋아할 만한 영화를 **얼마나 상단(앞쪽)에 배치했는지**를 평가합니다. 앞쪽에 있을수록 더 높은 점수를 부여합니다.
- **첫 관련 항목 순위 (MRR@K)**: 사용자가 좋아할 만한 영화가 **평균적으로 몇 번째 순서에 등장하는지**를 역수로 계산한 지표입니다. 1에 가까울수록 좋습니다.

### 2. 비즈니스 및 다양성 지표 (추천 리스트가 지루하지 않고 풍성한가?)
- **아이템 노출 범위 (Catalog Coverage)**: 시스템이 가진 전체 영화 중 **실제로 추천에 한 번이라도 등장한 영화의 비율**입니다. 이 수치가 높을수록 특정 인기 영화에만 추천이 쏠리지 않는다는 뜻입니다.
- **추천 다양성 (Intra-List Diversity)**: 한 명의 사용자에게 추천된 리스트 안에서 영화들이 **얼마나 서로 다른 장르와 특징을 가졌는지** 측정합니다. 너무 비슷한 영화만 나오면 지루할 수 있기 때문에 중요합니다.
- **참신함 (Novelty)**: 사용자가 이미 알고 있을 법한 너무 유명한 영화보다는, **몰랐을 법한 새로운 영화를 얼마나 발굴해주느냐**를 나타냅니다.
- **개인화 정도 (Personalization)**: 사용자마다 **얼마나 서로 다른 리스트를 받고 있는지**를 측정합니다. 모든 사람에게 똑같은 인기 영화만 추천해준다면 이 점수가 낮아집니다.
...
- [프로젝트 회고 및 성능 개선 보고서](docs/project_retrospective.md)
